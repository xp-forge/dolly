...
class Update(Visitor):
	...
	def visit(self, host):
		...
		if host.post_update and self.run_post_update:
			self.post_update_pool.apply_async(util.executeCommand, (host.post_update, dolly.Dolly.rootdir, True, '{0} Project Post Update'.format(host.name)))
...
class UpdateImpl(object):
	...
	def runPostUpdateCommand(self, repo):
		...
		if repo['post_update'] != '' and self.run_post_update:
			util.executeCommand(repo['post_update'], cwd=repo['local'], tmux=True, name='{0} Repository Post Update'.format(repo['name']))
